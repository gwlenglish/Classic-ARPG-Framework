...
{
    public class QuestgiverCreateEditor: ArpgWithCacheEditor
    {
        public override void OnInspectorGUI()
        {
            var model = (CreateQuestgiverEditorModel) target;
            var questGiverOptions = model.options;
            ...
            ArpgEditorHelper.DrawPopupWithLabel(ref questGiverOptions.Attributes, "Attributes", model.AttributesOptions);
            ArpgEditorHelper.DrawInlineEditorInFoldout(
                model.GameDatabase.Attributes.GetDatabaseObjectBySlotIndex(questGiverOptions.Attributes),
                ref model.AttributesOptionsExpanded, "Attributes View", EditorCache, true);
            EditorGUILayout.Space(5);
            
            ArpgEditorHelper.DrawHorizontalToggleField(ref questGiverOptions.UseBuiltInCanvas, "Use Builtin Canvas?");
            ArpgEditorHelper.DrawHorizontalObjectField(ref questGiverOptions.CharacterMesh, "Character Mesh?");
            
            if (GUILayout.Button("Create"))
            {
                string assetname = model.AttributesOptions[questGiverOptions.Attributes] + "_Prefab";
                ...
                UnityEngine.Object returnedAsset = QuestGiverDefinition.CreateQuestGiverPrefab(folder, questGiverOptions, model.GameDatabase);
