...
namespace GWLPXL.ARPGCore.States.com
{
    ...
    [CreateAssetMenu(menuName = "GWLPXL/ARPG/States/Player/2D/Animation/Walk_Down")]

    public class WalkDown_2D : PlayerMovementState2D
    {
        ...
        public override bool GetTransition(IStateMachineEntity forEntity)
        {
            ...
            {
                return forEntity.GetActorHub().InputHub.MoveInputs.GetVerticalRaw() < 0 && forEntity.GetActorHub().InputHub.MoveInputs.GetHorizontalRaw() == 0;
            }
            else
            {
